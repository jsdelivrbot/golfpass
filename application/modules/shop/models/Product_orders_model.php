...
class Product_orders_Model extends Board_Model{
    ...
    public function gets($where_obj =null,$select_arr =false,$limit=null)
    {
        ...
        $this->db->select("o.*, o.order_name '주문명',o.total_price '주문금액',(CASE WHEN o.pay_method = 'vbank' THEN '가상계좌' WHEN o.pay_method = 'card' THEN '카드' END) '결제방식', is_review_write as '후기작성 여부'");
        $this->db->from("$this->table AS o");
