<table class="ui selectable table">
    ...
    <tbody>
        ...
        <tr>
            <td class="selectable">
                <a href="<?=my_site_url(admin_package_uri."/update/{$products[$i]->id}")?>">
                    <?=$products[$i]->id?>
                </a>
            </td>
            <td class="selectable">
            ...
            <a href="#" onclick="confirm_redirect('<?=my_site_url(admin_ref_cate_product_uri."/goToRecycleBin/{$products[$i]->id}")?>','휴지통으로 이동시키겠습니까? 연결된 카테고리가 모두 삭제됩니다..'); return false;">
                ...
                /
                <a onclick="confirm_redirect('<?=my_site_url(admin_package_uri."/delete/{$products[$i]->id}")?>','정말 삭제하시겠습니까? 복구 할 방법이 없습니다.')" href="#">삭제</a>
            </td>
...
<a class="ui button basic positive" href="<?=my_site_url(admin_package_uri.'/add')?>">상품추가</a>

<form class="ui form" style="margin-top:30px;" class="" action="<?=my_site_url(admin_package_uri."/gets")?>" method="post">
    <div class="fields">
        <div class="field">
