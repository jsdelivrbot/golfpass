...
class Sample extends Base_Controller {
    ...
    function get($id)
    {
        $data['sample'] = $this->{$this->model}->_get($id);
    }
    function gets()
    {
        // $data['samples'] = $this->{$this->model}->_gets_with_pgi(null,'style_1');
        $data['samples'] = $this->{$this->model}->_gets();
    ...
    function add()
    {
        ...
        else
        {
            ...
            my_redirect(sample_uri."/update/$insert_id");
